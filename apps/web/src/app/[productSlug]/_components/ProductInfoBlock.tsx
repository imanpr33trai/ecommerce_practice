...
const StarRatingDisplay = ({
	...
	<div className="flex items-center gap-1">
		...
		<span className="ml-2 font-medium text-neutral-300 text-sm">
...
export const ProductInfoBlock = ({ product }: ProductInfoBlockProps) => {
	...
	const {
		mutate: cartMutations,
		isPending: isAddingToCart,
		isError: cartError,
	} = useCart.addToCart();
	const {
		mutate: wishMutations,
	...
	return (
		<div className="space-y-6 rounded-2xl bg-neutral-800 p-4 shadow-xl lg:p-6">
			{/* Product Name & Price */}
			<div>
				<h1 className="font-extrabold text-3xl text-white lg:text-4xl">
					{product.name}
				</h1>
				<p className="mt-2 font-bold text-white text-xl lg:text-3xl">
				...
				>
					{["S", "M", "L"].map((size) => (
						<Label
							...
							htmlFor={size}
							className={cn(
								"flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border bg-neutral-700 font-medium text-sm text-white transition-colors hover:bg-neutral-600",
								selectedSize === size
			...
			<div>
				...
				<div className="mt-2 flex h-10 items-center rounded-lg border border-neutral-600 bg-neutral-700">
					<Button
						variant="ghost"
						size="icon"
						className="h-full rounded-r-none border-neutral-600 border-r hover:bg-neutral-600"
					...
					<Input
						...
						className="h-full flex-1 border-x-0 border-y-0 bg-transparent text-center text-white [-moz-appearance:_textfield] focus-visible:ring-0 focus-visible:ring-offset-0 [&::-webkit-inner-spin-button]:m-0 [&::-webkit-outer-spin-button]:m-0"
					/>
					<Button
						variant="ghost"
						size="icon"
						className="h-full rounded-l-none border-neutral-600 border-l hover:bg-neutral-600"
			...
			<div className="flex flex-col gap-3">
				...
				<Button
					className="w-full rounded-lg bg-neutral-700 py-2.5 text-white hover:bg-neutral-600"
