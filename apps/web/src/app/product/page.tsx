...
export default function ProductsPage() {
	...
	const { data: PRODUCTS, isError, error } = Product.hooks.list(filters);

	if (isError || error || PRODUCTS === undefined || PRODUCTS === null) {
		console.log("the product is undefinded");
		return;
	}

	// const filteredProducts = PRODUCTS.items.filter((p) => {
	// 	if (filters.categories.length === 0) {
	// 		if (categoryParam !== "All" && p.category?.name !== categoryParam) {
	// 			return false;
	// 		}
	// 	} else {
	// 		if (!filters.categories.includes(p.category?.name)) {
	// 			return false;
	// 		}
	// 	}

	// 	if (isSaleParam && !p.isOnSale) {
	// 		return false;
	// 	}
	// 	if (filters.onSale && !p.isOnSale) {
	// 		return false;
	// 	}
	// 	if (filters.inStock && !p.isOnSale) {
	// 		return true;
	// 	}
	...
	// 		if (!p.material || !filters.materials.includes(p.material)) {
	// 			return false;
	// 		}
	// 	}
	...
	return (
		<div className="p-4 md:px-8 max-w-[1600px] mx-auto animate-fade-in relative">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
				<ProductCard product={PRODUCTS.items} />
