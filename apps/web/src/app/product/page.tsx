...
export default function ProductsPage() {
  ...
  const { data: PRODUCTS, isError, error } = useProduct.allProducts();
  ...
  const filteredProducts = PRODUCTS.filter((p) => {
    if (filters.categories.length === 0) {
      ...
      if (!filters.categories.includes(p.category?.name)) return false;
    }
      ...
      <div className="flex justify-between items-end mb-8 relative z-20">
        ...
        <div className="flex gap-2 relative">
          ...
          >
            ...
            <div
              className={`absolute top-full right-0 mt-2 z-[60] origin-top-right transition-all duration-300 ease-out transform ${
                showFilters
