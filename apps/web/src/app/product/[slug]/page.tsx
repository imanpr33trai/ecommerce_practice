        ...
        <ArrowLeft size={16} /> Back to Collection
			...
			<div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-16">
				<BentoCard className="lg:col-span-8 min-h-[500px] lg:h-[650px] bg-[#F4F4F4] relative group overflow-hidden">
					...
					<button onClick={handleWishlist} className="absolute top-6 right-6 p-3 rounded-full bg-white/90 backdrop-blur shadow-md transition-all duration-300 hover:scale-110 active:scale-95">
						<Heart size={24} className={`transition-colors duration-300 ${isWishlisted ? "fill-red-500 text-red-500" : "text-gray-700"}`} />
					</button>
					...
					<BentoCard className="p-6 bg-white">
						...
						<div className="flex gap-3 mb-8">
							{(product.colors || ["#D9D9D9", "#3A3A3A", "#8C7A6B"]).map((color, idx) => (
								<button key={idx} onClick={() => setActiveColor(color)} className={`w-10 h-10 rounded-full border-2 transition-all duration-200 ${activeColor === color ? "border-black scale-110" : "border-transparent"}`} style={{ backgroundColor: color }} title={color} />
							))}
						</div>
