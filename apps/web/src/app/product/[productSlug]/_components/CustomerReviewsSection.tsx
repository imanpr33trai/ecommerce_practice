...
import type { ProductDetailed, ProductReview } from "@/utils/typesClient";
...
const AddReviewForm = ({
	...
	const { mutate: reviewMutations, isPending: isSubmitting } =
		useReview.addReview(); // Assuming useReview hook provides addReview mutation
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<div className="grid gap-1.5">
						...
						<Textarea
							...
							minLength={10}
							disabled={isSubmitting}
							className="min-h-[100px] border-gray-600/50 bg-white/10 text-white placeholder:text-gray-400 focus:border-blue-500 focus:ring-blue-500"
						/>
					</div>
					<Button
						type="submit"
...
export const CustomerReviewsSection = ({
	...
	const { data: reviews, isLoading: areReviewsLoading } =
		useReview.reviewsByProductId(product.id);
			...
			<CardContent className="space-y-6">
				{areReviewsLoading ? (
					...
					<div className="space-y-6">
						{reviews.map(
							(
								review: ProductReview, // Cast to ProductReview type
							) => (
								<div
									key={review.id}
									className="rounded-lg border border-neutral-600/50 bg-neutral-700/50 p-4"
								>
