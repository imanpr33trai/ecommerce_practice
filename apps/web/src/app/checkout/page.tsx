...
export default function CheckoutPage() {
  ...
  const handleSelectAddress = (id: string) => {
    ...
    // Optional: Pre-fill form if needed, or clear it to indicate selection
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    if (!finalAddressId) {
      try {
        const newAddress = await createAddress({
          ...
          city: formData.city,
          state: "NY", // Hardcoded for MVP or add input
        ...
        // Note: Check your useAddAddress hook implementation to ensure it returns the data
        // If standard tRPC mutation, use onSuccess to capture, or update backend to return object.
        // Assuming backend returns the created object:
        // finalAddressId = newAddress.id;

        // Fallback for MVP if hook doesn't return data directly (optimistic approach):
        toast.info("Address saving...");
        // In a real scenario, ensure createAddress returns the object.
        // For now, we will stop here if logic requires ID.
        return;
      } catch (error) {
    ...
    // Proceed to checkout
    placeOrder({
      addressId: finalAddressId || addresses?.[0]?.id || "", // Fallback logic
      paymentProvider: "stripe",
    });
  };

  // --- REDIRECTS ---
  // Ensure we are in browser environment before pushing
  useEffect(() => {
  ...
  if (cartItems.length === 0) return null; // Avoid flash of content before redirect
    ...
    <div className="p-4 md:px-8 max-w-[1400px] mx-auto animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          {/* STEP 1: SHIPPING */}
          <BentoCard className={`p-8 bg-white transition-opacity duration-300 ${step === 2 ? "opacity-50 pointer-events-none grayscale" : "opacity-100"}`}>
          ...
          {/* STEP 2: PAYMENT */}
          <BentoCard className={`p-8 bg-white transition-all duration-300 ${step === 1 ? "opacity-50 pointer-events-none" : "opacity-100 ring-2 ring-black"}`}>
