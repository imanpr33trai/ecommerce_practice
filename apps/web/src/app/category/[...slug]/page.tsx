...
import { Star, Truck, ShieldCheck, ArrowLeft, Heart, Send, Ruler, Droplets, PenTool } from 'lucide-react';
import BentoCard from '@/components/ui/BentoCard';
import ProductCard from '@/components/ProductCard';
import Button from '@/components/ui/Button';
import Breadcrumbs from '@/components/Breadcrumbs';
import { PRODUCTS, PRODUCT_REVIEWS } from '@/constants';
...
export default async function CategoryPage({
  ...
  // const slugArray = (await params).id ?? [];
  const {id} = await params

  // if no slug segments, treat as not found (or handle root-category case)
  ...
  // // Use the direct server caller to fetch data on the server (avoids invalid invocation)
  // const categoryData = await api.category.getBySlug({ slug: currentSlug });
  // if (!categoryData) return notFound();

  // // Seed the server QueryClient cache so the client can hydrate without refetch
  // queryClient.setQueryData(
  //   trpc.category.getBySlug.queryKey({ slug: currentSlug }),
  //   categoryData
  // );

  // const { data: categoryData } = useCategory.getBySlug(currentSlug);

  // if (!categoryData) {
  //   console.log("the categorydata is undefinded");
  //   return;
  // }
...
const { toggleCart } = useContext(LayoutContext);
  ...
  const [activeColor, setActiveColor] = useState(product.colors?.[0] || '#D9D9D9');
  ...
  return (
    <div className="p-4 md:px-8 max-w-[1600px] mx-auto pb-12 animate-fade-in">
      <Breadcrumbs />
      <Link href="/products" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-black mb-6">
        <ArrowLeft size={16} /> Back to Collection
      </Link>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-16">
        <BentoCard className="lg:col-span-8 min-h-[500px] lg:h-[700px] bg-[#F4F4F4] relative group overflow-hidden">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          ...
          <BentoCard className="p-6 bg-white">
            ...
            <div className="space-y-3">
              ...
              <Button variant="outline" className="w-full">Download Spec Sheet</Button>
            </div>
          </BentoCard>
      ...
      <div className="mb-16 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div className="space-y-6 px-4">
          ...
          <h2 className="text-4xl font-light leading-tight">Crafted with intention. <br/> Built for longevity.</h2>
          <p className="text-gray-600 text-lg leading-relaxed">
            The {product.name} isn't just a piece of furniture; it's a statement of minimalist philosophy.
            Designed in our Stockholm studio, every curve serves a purpose. We sourced the finest materials
            to ensure that it doesn't just look good on day one, but develops a rich patina over years of use.
          </p>
          <div className="grid grid-cols-2 gap-4 pt-4">
            <div>
              <h4 className="font-bold text-2xl mb-1">100%</h4>
              <p className="text-xs text-gray-500 uppercase tracking-wider">Sustainable Wood</p>
            </div>
             <div>
              <h4 className="font-bold text-2xl mb-1">300+</h4>
              <p className="text-xs text-gray-500 uppercase tracking-wider">Hours of Craft</p>
            </div>
          </div>
        </div>
        <div className="h-[500px] rounded-[2rem] overflow-hidden">
          <img src="https://images.unsplash.com/photo-1581539250439-c96689b516dd?auto=format&fit=crop&q=80&w=1200" alt="Craftsmanship" className="w-full h-full object-cover" />
        </div>
      </div>
