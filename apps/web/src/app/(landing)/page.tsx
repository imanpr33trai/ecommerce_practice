...
const ProductSlider: React.FC<ProductSliderProps> = ({ title, subtitle, products, categoryLink }) => {
	...
	return (
		<div className="space-y-6 animate-slide-up relative group" onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
			...
			<Button variant="icon" onClick={() => scroll("left")} className="absolute left-0 top-[55%] -translate-y-1/2 z-20 -ml-5 shadow-xl bg-white text-black border border-gray-100 opacity-0 group-hover:opacity-100 transition-all duration-300 hidden md:flex hover:scale-110">
				<ArrowLeft size={20} />
			</Button>

			<Button variant="icon" onClick={() => scroll("right")} className="absolute right-0 top-[55%] -translate-y-1/2 z-20 -mr-5 shadow-xl bg-white text-black border border-gray-100 opacity-0 group-hover:opacity-100 transition-all duration-300 hidden md:flex hover:scale-110">
				<ArrowRight size={20} />
			</Button>

			<div ref={scrollRef} className="flex gap-4 overflow-x-auto no-scrollbar pb-8 pt-4 snap-x p-2 -mx-2 scroll-smooth items-start" style={{ scrollPaddingLeft: "1rem", scrollPaddingRight: "1rem" }}>
				{products.map((product) => (
					<div key={product.id} className="min-w-[300px] md:min-w-[340px] snap-center shrink-0">
...
export default function HomePage() {
	...
	const livingRoomProducts = allProducts.filter((p) => ["Sofa", "Chair", "Bed"].includes(p.category?.name || "not"));
	const workspaceProducts = allProducts.filter((p) => ["Table", "Lamps", "Dressers"].includes(p.category?.name || "not"));
		...
		<div className="p-4 md:px-8 pb-8 space-y-16 max-w-[1600px] mx-auto animate-fade-in">
					...
					<div className="relative z-10 flex flex-col md:flex-row items-center justify-around h-full p-8 md:p-12 gap-8">
						<div className="flex-1 space-y-6 max-w-md">
							...
							<div className="flex gap-3 pt-4">
								...
								<Button variant="icon" className="bg-white hover:bg-white/80 h-12 w-12 transition-transform active:scale-90" onClick={() => setIsHeroWishlisted(!isHeroWishlisted)}>
									<Heart size={20} className={`transition-colors duration-300 ${isHeroWishlisted ? "fill-red-500 text-red-500" : "text-black"}`} />
								</Button>
							</div>
						</div>
						...
						<div className="flex-1 relative w-full max-w-[400px] aspect-square">
							...
							<img src={featuredProduct.image} alt={featuredProduct.name} className="relative w-full h-full object-contain drop-shadow-2xl transition-transform duration-700 ease-out group-hover:scale-105" />
						</div>
					</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				{[
					...
					<BentoCard key={idx} className="p-6 flex items-center gap-4 bg-white">
			...
			{recentlyViewed.length > 0 && <ProductSlider title="Recently Viewed" subtitle="Pick up where you left off." products={recentlyViewed} categoryLink="/products" />}

			<div className="grid grid-cols-1 md:grid-cols-3 gap-4 min-h-[400px]">
				<BentoCard className="md:col-span-2 relative group overflow-hidden bg-[#E8E8E6]">
					<img src="https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&q=80&w=1200" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
					...
					<div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur px-6 py-4 rounded-3xl">
						...
						<Link href="/products?category=Sofa" className="text-xs font-bold uppercase tracking-wider hover:underline flex items-center gap-1">
				...
				<div className="flex flex-col gap-4">
					<BentoCard className="flex-1 relative group overflow-hidden bg-white">
						<img src="https://images.unsplash.com/photo-1507473888900-52e1ad14db3d?auto=format&fit=crop&q=80&w=600" className="absolute inset-0 w-full h-full object-cover opacity-90 transition-transform duration-700 group-hover:scale-110" />
						<div className="absolute top-4 left-4 bg-white px-3 py-1 rounded-full text-xs font-bold shadow-sm">Lighting</div>
						<Link href="/products?category=Lamps" className="absolute inset-0" />
					</BentoCard>
					<BentoCard className="flex-1 relative group overflow-hidden bg-white">
						<img src="https://images.unsplash.com/photo-1598300042247-d088f8ab3a91?auto=format&fit=crop&q=80&w=600" className="absolute inset-0 w-full h-full object-cover opacity-90 transition-transform duration-700 group-hover:scale-110" />
						<div className="absolute top-4 left-4 bg-white px-3 py-1 rounded-full text-xs font-bold shadow-sm">Chairs</div>
						<Link href="/products?category=Chair" className="absolute inset-0" />
					</BentoCard>
				</div>
			</div>

			<ProductSlider title="Living Room Comfort" subtitle="Relax in style with our premium sofas and chairs." products={livingRoomProducts} categoryLink="/products?category=Sofa" />
			<ProductSlider title="Workspace & Lighting" subtitle="Illuminate your ideas with our curated collection." products={workspaceProducts} categoryLink="/products?category=Table" />
				...
				<BentoCard className="md:col-span-4 p-8 bg-[#C6BAA8] text-black flex flex-col justify-between group cursor-pointer relative overflow-hidden">
					...
					<div className="relative z-10 flex items-center justify-between mt-8">
						<div className="flex -space-x-3">
							{TEAM.map((member) => (
								<img key={member.id} src={member.image} className="w-10 h-10 rounded-full border-2 border-[#C6BAA8] object-cover" />
							))}
						</div>
