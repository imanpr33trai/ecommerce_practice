...
import { ArrowRight, ArrowLeft, ArrowUpRight, Heart, Star, Truck, ShieldCheck, RefreshCcw } from 'lucide-react';
import Link from 'next/link';
import BentoCard from '@/components/ui/BentoCard';
import ProductCard from '@/components/ProductCard';
import Button from '@/components/ui/Button';
import { PRODUCTS, TEAM, REVIEWS } from '@/constants';
    ...
    >
      <div className="flex justify-between items-end px-2">
        ...
        <Link href={{pathname: categoryLink}}>
          <Button variant="outline" size="sm" className="rounded-full">View Collection</Button>
        </Link>
      </div>
...
export default function HomePage() {
  ...
  const newDeals = PRODUCTS.filter(p => p.isNew);
  const featuredProduct = newDeals[currentSlide % newDeals.length] || newDeals[0];

	if(featuredProduct
		===undefined
	) {
		console.log("featured product is undefined");
		return
	}
  ...
  const livingRoomProducts = PRODUCTS.filter(p => ['Sofa', 'Chair', 'Bed'].includes(p.category));
  const workspaceProducts = PRODUCTS.filter(p => ['Table', 'Lamps', 'Dressers'].includes(p.category));
    ...
    <div className="p-4 md:px-8 pb-8 space-y-16 max-w-[1600px] mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-auto lg:h-[600px]">
        <BentoCard className="lg:col-span-8 relative bg-[#F2F2F0] flex flex-col justify-center overflow-hidden group">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[12vw] font-bold text-white uppercase tracking-tighter leading-none select-none">
            Nestify
          </div>
          ...
          <div className="absolute bottom-6 right-6 flex gap-2">
             <Button variant="secondary" size="icon" onClick={prevSlide}><ArrowLeft size={20} /></Button>
             <Button variant="primary" size="icon" onClick={nextSlide}><ArrowRight size={20} /></Button>
          </div>
        </BentoCard>
        ...
        <BentoCard className="lg:col-span-4 bg-black text-white p-8 flex flex-col justify-between relative overflow-hidden group">
           <div className="relative z-10">
              ...
              <h2 className="text-4xl font-light mt-4 mb-2">Summer <br/>Sale</h2>
           ...
           <div className="relative z-10 mt-8">
              <Link href={{
                ...
                query:"sale=true",
                 ...
                 <Button variant="secondary" className="w-full justify-between group-hover:pl-8">Shop Sale <ArrowRight size={16} /></Button>
              </Link>
           </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 min-h-[400px]">
         <BentoCard className="md:col-span-2 relative group overflow-hidden bg-[#E8E8E6]">
            ...
            <div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur px-6 py-4 rounded-3xl">
               ...
               <Link href={{
                pathname:"/product",query:"category=Sofa"
               }} className="text-xs font-bold uppercase tracking-wider hover:underline flex items-center gap-1">Explore Collection <ArrowRight size={12} /></Link>
            </div>
         </BentoCard>
         <div className="flex flex-col gap-4">
            <BentoCard className="flex-1 relative group overflow-hidden bg-white">
                ...
                <Link href={{
                pathname:"/product",query:"category=Lamps"
            ...
            <BentoCard className="flex-1 relative group overflow-hidden bg-white">
                ...
                <Link href={{
                pathname:"/product",query:"category=Chair"
               }} className="absolute inset-0" />
            </BentoCard>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
         ...
         <BentoCard className="md:col-span-4 p-8 bg-[#C6BAA8] text-black flex flex-col justify-between group cursor-pointer relative overflow-hidden">
             <div className="relative z-10">
                ...
                <h3 className="text-3xl font-medium mt-4 leading-tight">Join the <br/>craft.</h3>
