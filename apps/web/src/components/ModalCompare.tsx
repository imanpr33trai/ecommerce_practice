...
const ModalCompare: React.FC = () => {
	...
	return (
		<div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
			<div className="absolute inset-0 bg-black/40 backdrop-blur-sm animate-fade-in" onClick={() => setCompareOpen(false)} />
			<div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-6xl h-[85vh] overflow-hidden animate-slide-up flex flex-col">
				<div className="flex items-center justify-between p-6 border-b border-gray-100">
					...
					<button onClick={() => setCompareOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
						<X size={20} />
					</button>
				</div>
					...
					<div className="flex-1 overflow-auto p-8">
						<div className="grid grid-cols-1 md:grid-cols-4 min-w-[800px]">
							...
							{compareList.map((product) => (
								<div key={product.id} className="col-span-1 border-l border-gray-100 px-6 first:border-l-0 relative group">
									<Button onClick={() => removeFromCompare(product.id)} variant={"outline"} className="absolute top-0 right-6 p-2 z-1000 group bg-gray-100 rounded-full hover:bg-red-50 hover:text-red-500 transition-colors">
										<Trash2 size={16} className="" />
									</Button>

									<div className="h-[280px] bg-gray-50 rounded-2xl mb-8 overflow-hidden">
										<Image alt={product.name} width={100} height={100} src={product.image} className="w-full h-full object-cover mix-blend-multiply" />
									</div>
									...
									<div className="space-y-8">
										...
										<div className="h-12 flex items-center pt-4">
											<Button className="w-full text-sm" onClick={() => addToast("Added to cart")}>
												Add to Cart
											</Button>
										</div>
									</div>
