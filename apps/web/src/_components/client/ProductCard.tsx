  ...
  return (
    <Card className="group hover:-translate-y-2 relative w-full overflow-hidden rounded-3xl border border-white/20 bg-white/80 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] backdrop-blur-xl transition-all duration-500 hover:bg-white/90 hover:shadow-[0_8px_40px_0_rgba(31,38,135,0.25)] dark:bg-gray-900/80 dark:hover:bg-gray-900/90">
      ...
      <div className="relative z-10">
        ...
        {/* New/Hot badge */}
        {item.createdAt && <div className="absolute top-4 left-4 z-30 rounded-2xl border border-purple-400/30 bg-gradient-to-r from-purple-500/80 to-pink-500/80 px-3 py-1.5 font-bold text-white text-xs shadow-lg backdrop-blur-md">NEW</div>}
          ...
          <CardContent className="p-0">
            ...
            <div className="relative aspect-square overflow-hidden rounded-t-3xl bg-gradient-to-br from-gray-100 to-gray-50 dark:from-gray-800 dark:to-gray-900">
              ...
              <Image
                ...
                width={500}
                height={500}
                className={`h-full w-full object-cover transition-all duration-1000 ease-out group-hover:rotate-1 group-hover:scale-110 ${secondaryImage ? "group-hover:opacity-0" : ""}`}
                priority={false}
              />
            ...
            <div className="relative space-y-3 bg-gradient-to-b from-transparent to-white/50 p-6 backdrop-blur-sm dark:to-black/30">
              ...
              <div className="relative space-y-1">
                ...
                {item.category && <p className="font-medium text-gray-600 text-xs uppercase tracking-wider dark:text-gray-400">{item.category.name}</p>}
              </div>

              {/* Glassmorphic Price section */}
              <div className="flex items-baseline gap-2">
                <span className="bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text font-black text-2xl text-transparent dark:from-white dark:to-gray-300">${Number(item.price).toFixed(2)}</span>
                {item.price && item.price > item.price && <span className="text-gray-500 text-sm line-through dark:text-gray-400">${Number(item.price).toFixed(2)}</span>}
              </div>
              ...
              >
                {item.stock ? (
                  <span className="flex items-center justify-center">
                    ...
                    {isAddingCart ? <span className="animate-pulse">Adding...</span> : "Add to Cart"}
                  </span>
                ) : (
                  <span className="flex items-center justify-center">Out of Stock</span>
                )}
              </Button>

              {/* Glassmorphic Stock indicator */}
              {item.stock && item.stock <= 5 && !item.stock && <div className="rounded-xl border border-orange-200/30 bg-orange-100/50 py-1.5 text-center font-semibold text-orange-600 text-xs backdrop-blur-sm dark:bg-orange-900/30 dark:text-orange-400">🔥 Only {item.stock} left in stock</div>}
            </div>
          </CardContent>
        </Link>
      </div>
